...
@tasks_bp.route('/label_preview_tool')
@login_required
@manager_required
def label_preview_tool():
    """Herramienta interactiva para previsualizar y ajustar el tamaño de las etiquetas"""
    try:
        # Recuperar la configuración guardada si existe
        label_config = session.get('label_config', {
            'width': 35,
            'height': 40,
            'showSecondary': True,
            'spacing': 1
        ...
        return render_template('tasks/label_preview_tool.html', config=label_config)
    except Exception as e:
        flash(f'Error al cargar la herramienta de vista previa: {str(e)}', 'danger')
        return redirect(url_for('tasks.manage_labels'))

@tasks_bp.route('/save_label_config', methods=['POST'])
@login_required
@manager_required
def save_label_config():
    """Guardar la configuración de etiquetas"""
    try:
        data = request.json
        
        # Validar datos
        if not isinstance(data, dict):
            return jsonify({'success': False, 'error': 'Formato de datos inválido'})
        
        # Validar campos necesarios
        required_fields = ['width', 'height', 'showSecondary', 'spacing']
        for field in required_fields:
            if field not in data:
                return jsonify({'success': False, 'error': f'Falta el campo {field}'})
        
        # Validar tipos de datos
        if not isinstance(data['width'], int) or not (20 <= data['width'] <= 100):
            return jsonify({'success': False, 'error': 'Ancho inválido (debe ser entre 20mm y 100mm)'})
        
        if not isinstance(data['height'], int) or not (20 <= data['height'] <= 100):
            return jsonify({'success': False, 'error': 'Alto inválido (debe ser entre 20mm y 100mm)'})
        
        if not isinstance(data['showSecondary'], bool):
            return jsonify({'success': False, 'error': 'Valor inválido para mostrar caducidad secundaria'})
        
        if not isinstance(data['spacing'], (int, float)) or not (0 <= data['spacing'] <= 10):
            return jsonify({'success': False, 'error': 'Espaciado inválido (debe ser entre 0mm y 10mm)'})
        
        # Actualizar base de datos con la configuración global
        # Aquí puedes guardar en alguna tabla de configuración si lo deseas
        
        # Por ahora, guardar en la sesión para persistencia temporal
        session['label_config'] = {
            'width': data['width'],
            'height': data['height'],
...
@tasks_bp.route('/print_preview')
@login_required
@manager_required
def print_preview():
    """Generar una vista previa de impresión con la configuración actual"""
    try:
        # Obtener parámetros
        width = request.args.get('width', 35, type=int)
        height = request.args.get('height', 40, type=int)
        has_secondary = request.args.get('secondary', 'true').lower() == 'true'
        spacing = request.args.get('spacing', 1, type=float)
        
        # Obtener datos de ejemplo para la vista previa
        user = current_user
        now = datetime.now()
        expiry_datetime = now + timedelta(hours=24)  # Ejemplo: 24 horas de caducidad
        secondary_expiry_date = now.date() + timedelta(days=7) if has_secondary else None  # 7 días de vida útil
        
        # Crear un producto y conservación de ejemplo
        product_example = {
            'name': 'PRODUCTO DE EJEMPLO'
        }
        
        conservation_type_example = {
            'name': 'REFRIGERACIÓN',
            'value': 'refrigeracion'
        }
        
        # Crear una plantilla con configuración personalizada
        template = {
            'titulo_x': 50, 'titulo_y': 15, 'titulo_size': 12, 'titulo_bold': True,
            'conservacion_x': 50, 'conservacion_y': 30, 'conservacion_size': 10, 'conservacion_bold': True,
            'preparador_x': 50, 'preparador_y': 45, 'preparador_size': 9, 'preparador_bold': False,
            'fecha_x': 50, 'fecha_y': 55, 'fecha_size': 9, 'fecha_bold': False,
            'caducidad_x': 50, 'caducidad_y': 70, 'caducidad_size': 10, 'caducidad_bold': True,
            'caducidad2_x': 50, 'caducidad2_y': 85, 'caducidad2_size': 9, 'caducidad2_bold': False,
        }
        
        # Retornar la plantilla con estilos personalizados
        return render_template(
            'tasks/print_preview.html',
            product=product_example,
            conservation_type=conservation_type_example,
            user={'name': user.first_name, 'last_name': user.last_name},
            now=now,
            expiry_datetime=expiry_datetime,
            secondary_expiry_date=secondary_expiry_date,
            quantity=1,
        ...
        flash(f'Error al generar la vista previa: {str(e)}', 'danger')
        return redirect(url_for('tasks.label_preview_tool'))